var lvtuanApp = angular.module('lvtuanApp.Ctrl', ['ionic','ngSanitize','ngFileUpload','listModule'])
...
//律师-普通用户-个人资料
lvtuanApp.controller("infoCtrl",function($scope,$http,$rootScope,$timeout,Upload){
	console.info("个人资料");
	//判断是否是律师
	if($rootScope.user_group_id == 1 || $rootScope.user_group_id == 2 && $rootScope.is_verified == 0){
		//普通用户个人信息
...
//律师的积分
lvtuanApp.controller("listscoreslaywerCtrl",function($scope, listHelper) {
	listHelper.bootstrap('/center/lawyer/score/list_scores', $scope);
})
...
//用户的工作 - 咨询 － 评价
lvtuanApp.controller("confirmCompletionCtrl",function($scope,$http,$rootScope,$stateParams,listHelper){
	listHelper.bootstrap('/center/question/waitforevaluation', $scope);
	 // /question/{id}/to_evaluate
...
lvtuanApp.controller("userorderAllCtrl",function($http,$scope,$state,$rootScope,httpWrapper){
	...
	$scope.remove = function(id,index){
		...
		$http.post('http://'+$rootScope.hostName+'/center/question/'+id+'/remove',
	            {
	            headers: {
	                'Content-Type': 'application/json' , 
	            	'Authorization': 'bearer ' + $rootScope.token,
