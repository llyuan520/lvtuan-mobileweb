...
lvtuanApp.controller("citypickerCtrl",function($state,$timeout,$http,$location,$scope,$rootScope,$anchorScroll){
    ...
    //获取 省份
	$scope.province = function(key,val){
		$(".citypicker .province").hide();
		console.info(key,val);
		$scope.province_key = key; 
		$scope.cities = val;
	...
	$scope.citie = function(val){
		...
		console.info($scope.province_key,val);
		$scope.items = {
			'city_id' : $scope.province_key,
			'region_id' : val
		}
		sessionStorage.setItem("citypicker", JSON.stringify($scope.items));
